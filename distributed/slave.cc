...
#include <iomanip>         // std::fixed, std::setprecision (GC 垃圾率输出)
...
#define GC_LOW_WATER   20   // 垃圾率 <  此值 → 阈值+1（数据很"干净"，放慢 GC 步调）
// 垃圾率 ∈ [GC_LOW_WATER, GC_HIGH_WATER) → 阈值不变（稳定期）

// ★ 动态阈值上下限（宏定义，防止阈值极端漂移）
#define GC_MIN_THRESHOLD  2
#define GC_MAX_THRESHOLD  128
...
//   write_pos:   <当前写入位置(字节)>
//   gc_threshold: <当前 GC 阈值>        — 动态阈值，根据垃圾率自适应调整
static std::string vlog_meta_path() { return g_dbname + "/VLOG_META"; }

// 当前生效的 GC 阈值（初始 = kGcVlogFileCountThreshold，可动态调整）
// ★ 动态调整依据：每次 GC 最旧的一个 vlog 文件，根据其垃圾率调整阈值
//   - 垃圾率 >= GC_HIGH_WATER → 阈值-1（加快 GC 步调，减少空间浪费）
//   - 垃圾率 <  GC_LOW_WATER  → 阈值+1（放慢 GC 步调，降低 CPU 开销）
//   - 垃圾率 ∈  [GC_LOW_WATER, GC_HIGH_WATER) → 阈值不变（稳定期）
//   阈值始终钳位在 [GC_MIN_THRESHOLD, GC_MAX_THRESHOLD] 区间内
...
static int execute_gc_on_file(uint64_t vlog_number, bool check_stop,
                               int* out_total, int* out_valid);
...
std::string process_command(const std::string& line) {
  ...
  // 若从节点只有一个 vlog 文件（必定为当前活跃的热文件，不可 GC），返回 GC DENY。
    ...
    // ★ 只取最旧的一个非活跃 vlog 文件
    uint64_t target = all_targets.front();
    uint64_t file_size = get_file_size(leveldb::LogFileName(g_dbname, target));

    std::cout << "Worker " << worker_id << ": 手动 GC 开始! 目标文件=" << target
              << ", 大小=" << (file_size >> 20) << "MB" << std::endl;

    // ★ check_stop=false：不检查退出标志，确保客户端 GC 操作完成
    //    out_total/out_valid=nullptr：手动 GC 不需要垃圾率统计
    int removed = execute_gc_on_file(target, false, nullptr, nullptr);
...
bool init_vlog(const std::string& dbname) {
  ...
  // 确保阈值在合法区间内（防御旧版 VLOG_META 或损坏数据）
  if (g_gc_threshold < GC_MIN_THRESHOLD)
    g_gc_threshold = GC_MIN_THRESHOLD;
  if (g_gc_threshold > GC_MAX_THRESHOLD)
    g_gc_threshold = GC_MAX_THRESHOLD;
...
// 对指定 vlog 文件执行 GC：读取 → 验证 → 搬迁有效值 → 条件删除
//
// ★ GC 验证规则（逐记录）:
...
//
// 参数:
//   vlog_number — 目标 vlog 文件编号（必须为非活跃文件，即 < g_vlog_number）
//   check_stop  — 如果为 true，循环中检查 g_running && g_gc_running（后台 GC 用）
//                  如果为 false，不检查退出标志（客户端手动 GC 用，保证操作完成）
//   out_total   — 输出参数：文件中的总记录数（可为 nullptr）
//   out_valid   — 输出参数：仍有效的记录数（可为 nullptr）
//
// 返回值: 1 = 成功删除文件, 0 = 文件保留（仍有有效记录）
static int execute_gc_on_file(uint64_t vlog_number, bool check_stop,
                               int* out_total, int* out_valid) {
  if (check_stop && (!g_running || !g_gc_running)) return 0;
  if (g_master_fd < 0) return 0;

  std::string old_vlog_path = leveldb::LogFileName(g_dbname, vlog_number);
  std::cout << "Worker " << worker_id << ":   GC 文件 " << vlog_number
            << " (" << old_vlog_path << ")" << std::endl;
  ...
  if (out_valid) *out_valid = valid_records;

  // 计算并打印垃圾率
  double garbage_rate = 0.0;
  if (total_records > 0) {
    garbage_rate = 100.0 * (total_records - valid_records) / total_records;
  }
  std::cout << "Worker " << worker_id << ":   GC 文件 " << vlog_number
            << " 完成! 总记录=" << total_records
            << ", 有效搬迁=" << valid_records
            << ", 垃圾率=" << std::fixed << std::setprecision(1) << garbage_rate << "%"
            << std::endl;

  // ★ 删除判定：搬迁完成后直接释放旧 vlog 文件磁盘空间
  //   （暂不考虑宕机，有效记录已全部搬迁到新 vlog，旧文件可立即删除）
  if (total_records > 0) {
...
// 收集非活跃 vlog 文件编号（旧→新排序，跳过当前活跃 vlog g_vlog_number）
static std::vector<uint64_t> collect_gc_targets(const std::vector<uint64_t>& vlog_files) {
...
// GC 线程主函数
//
// ★ GC 策略（单文件 GC + 垃圾率自适应阈值）:
//   1. 每 kGcIntervalSec 秒检查一次
//   2. 触发条件: 非活跃 vlog 文件数 >= g_gc_threshold（动态）
//   3. 每次仅取最旧的 1 个非活跃 vlog 文件执行 GC
//   4. GC 完成后计算该文件的垃圾率，据此动态调整阈值：
//      - 垃圾率 >= GC_HIGH_WATER（40%）→ 阈值-1（数据"脏"，加速 GC，减少空间浪费）
//      - 垃圾率 <  GC_LOW_WATER（20%） → 阈值+1（数据"干净"，放慢 GC，降低 CPU 开销）
//      - 垃圾率 ∈  [GC_LOW_WATER, GC_HIGH_WATER) → 阈值不变（稳定期）
//   5. 阈值始终钳位在 [GC_MIN_THRESHOLD, GC_MAX_THRESHOLD] 区间内
//
// ★ 并发安全:
...
static void* gc_thread_main(void*) {
  ...
  while (g_running && g_gc_running) {
    ...
    std::vector<uint64_t> gc_targets = collect_gc_targets(vlog_files);

    // ★ 动态阈值判断：若非活跃文件数低于当前阈值，无需 GC
    if (static_cast<int>(gc_targets.size()) < g_gc_threshold) {
      continue;
    }

    // ★ 每次仅 GC 最旧的一个非活跃 vlog 文件
    uint64_t target = gc_targets.front();
    uint64_t file_size = get_file_size(leveldb::LogFileName(g_dbname, target));
    ...
    int total = 0, valid = 0;
    int removed = execute_gc_on_file(target, true, &total, &valid);

    // ★ 垃圾率自适应阈值调整
    if (total > 0) {
      double garbage_rate = 100.0 * (total - valid) / total;
      int old_threshold = g_gc_threshold;

      if (garbage_rate >= GC_HIGH_WATER) {
        // 垃圾率高 → 数据"脏"，降低阈值以更频繁 GC，加速空间回收
        g_gc_threshold--;
        if (g_gc_threshold < GC_MIN_THRESHOLD) g_gc_threshold = GC_MIN_THRESHOLD;
      } else if (garbage_rate < GC_LOW_WATER) {
        // 垃圾率低 → 数据"干净"，升高阈值以降低 GC 频率，节省 CPU
        g_gc_threshold++;
        if (g_gc_threshold > GC_MAX_THRESHOLD) g_gc_threshold = GC_MAX_THRESHOLD;
      }
      // else: 垃圾率 ∈ [GC_LOW_WATER, GC_HIGH_WATER) → 阈值不变（稳定期）

      if (g_gc_threshold != old_threshold) {
        std::cout << "Worker " << worker_id << ": GC 阈值调整! "
                  << old_threshold << " → " << g_gc_threshold
                  << " (垃圾率=" << std::fixed << std::setprecision(1)
                  << garbage_rate << "%)" << std::endl;
      }
    }

    std::cout << "Worker " << worker_id << ": GC 本轮完成! 目标文件=" << target
              << ", 删除=" << (removed ? "是" : "否")
              << ", 总记录=" << total << ", 有效=" << valid << std::endl;
